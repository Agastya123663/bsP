...
export default class HomeScreen extends Component{
  ...
  getRequestedExchangeList =()=>{
    this.requestRef = db.collection("requested_items")
    .onSnapshot((snapshot)=>{
      var requestedExchangeList = snapshot.docs.map(document => document.data());
      this.setState({
        requestedExchangeList : requestedExchangeList
      });
    })
  }

  componentDidMount(){
    this.getRequestedExchangeList()
  }
  ...
  render(){
    return(
      <View style={{flex:1,backgroundColor:'#87ceeb'}}>
        ...
        <View style={{flex:1,backgroundColor:'#87ceeb'}}>
          {
            this.state.requestedExchangeList.length === 0
            ?(
            ...
            :(
              <FlatList
                ...
                data={this.state.requestedExchangeList}
